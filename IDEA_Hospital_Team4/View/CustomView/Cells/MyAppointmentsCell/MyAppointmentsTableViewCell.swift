...
class MyAppointmentsTableViewCell: UITableViewCell {
    ...
    func configure(myAppointment: MyAppointment, index: Int){
        ...
        self.reviewsCountLabel.text = "\(myAppointment.doctor?.reviewsCount ?? 0)" + " " + "Reviews"
...
extension MyAppointmentsTableViewCell{
    ...
    private func setupImage(){
        self.doctorImageView.cornerStylish(color: UIColor(hexString: "#001867"), borderWidth: 2, cornerRadius: doctorImageView.frame.width/2)
    ...
    private func setupViewMapButton() {
        ...
        viewOnMapBtn.setImage(Asset.pinSmall.image, for: .normal)
